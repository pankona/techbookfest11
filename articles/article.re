...
EM がやりやすい仕事のひとつとして、"中長期的に健全なチーム運営をしていくために脳みそを使う" というものがあると考えています。
...
コードを書いて何かを作る作業というのは、作ったそばからものを動くのを観測することができ、達成感を得やすい類に入ります。
一方、EM の仕事はどちらかと言えば "チームを健全に保つ" ことにあり、極端に言えば "何も問題が起こらないことが最も良い" とされる部分もありそうです (生産性を阻害する要因は無数にあるので、何も問題がないチームというのはないと思いますが)。

何も起こらないことに対して達成感を得るというのは、コードを書いて達成感を得てきた身からするとギャップが大きく、ややもすれば「自分は EM として何も仕事していないんじゃないか」等とも思いがちです。
...
これから EM になる他の方も似たような轍を踏む可能性があるのではないかと思い、もしかしたら同じ失敗を侵さぬような参考になれば幸いです。
...
いまの私から当時に私にアドバイスできるとすれば、"自分自身をプレイヤーとして頭数に数えないくらいで十分である" と助言するでしょう。

=== 垂れ流すコミュニケーション

EM になったばかりの頃の私は、"チームメンバーに対し、情報は良いことも悪いこともオープンにする" という信念でやっていこうと考えていました。これはいまでも変わっていません。
...
オープンにするのは良いと思いつつ、"いつ、だれに、何をインプットするのが効果的か" という観点が大事であると認識しました。
...
移譲することができない理由はだいたい分かっていて、私自身が "自分が聞いた事柄をふたたび誰かに説明をするのが面倒" と思っているためでしょう。
いずれにせよ、私が作業の移譲に失敗することはチームとしての仕事量がスケールしない結果をまねくことになります。いまだに苦手な部分ですが、改善していきたい点です。
...
計測できるものは計測し、より事実 (データ) に基づいた活動を行うことで、再現性のある "生産性のマネジメント" を行うことができるのではないかと画策しています。
...
これらがただちに生産性を表すメトリックスではないと思いますが、定点観測していくことで見えてくるものもあるのではないかと思っています。
これらのメトリックスを自動的に収集して Datadog に表示することができたら便利かもしれないですね。

=== 技術力を失わないために

EM はキャリアの終点ではなく、またプレイヤーに戻るタイミングがくると予想しています。
いつでも戻れるように、公私に渡って技術的なものに常に触れておくことを続けておこうと思っています。
...
これはつまり、EM の仕事を具体的に定義するのは難しいということを意味していて、やや曖昧な言い方をすれば「チームの生産性のボトルネックやメンバーのストレスに向き合い、解決に導くために脳みそを使う」というのが仕事になるのだと思います。
